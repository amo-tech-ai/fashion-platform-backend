...
export const get = api<GetPlanParams, FullProductionPlan>(
  ...
  async ({ planId }) => {
    ...
    return {
      ...
      attendeeMix: plan.attendee_mix,
      createdAt: plan.created_at,
      updatedAt: plan.updated_at,
      eventName: plan.event_name,
      eventDate: plan.event_date,
      eventStatus: plan.event_status,
      milestones: milestones.map(m => ({ ...m, planId: m.plan_id, dueDate: m.due_date, assignedTo: m.assigned_to, dependsOn: m.depends_on, durationDays: m.duration_days })),
